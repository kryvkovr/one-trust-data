...
import static com.rnonetrust.Utils.convertJsonToMap;
import static com.rnonetrust.Utils.toArray;
...
public class OneTrust extends ReactContextBaseJavaModule {
    ...
    public void getPreferenceCenterData(Promise promise) {
        try {
            JSONObject data = ot.getPreferenceCenterData();
            if(data == null){
    ...
    public void getBannerData(Promise promise) {
        try {
            JSONObject data = ot.getBannerData();
            if(data == null){
                throw new Exception("There is no preference center data");
            }
            promise.resolve(data.toString());
